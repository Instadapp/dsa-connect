...
export const getABI = async (abi_idx: number, answers: Obj): Promise<string> => {
  ...
  if (abi_idx === 0 && answers.chain === 'Mainnet') {
    const res = await fetch(
      `https://api.etherscan.io/api?module=contract&action=getabi&address=${answers.address}&apikey=${process.env.MAINNET_API_KEY_TOKEN}`
    )
    ...
    const res = await fetch(
      `https://api.polygonscan.com/api?module=contract&action=getabi&address=${answers.address}&apikey=${process.env.POLYGON_API_KEY_TOKEN}`
    )
    const { result } = await res.json()
    ...
    const res = await fetch(
      `https://api.arbiscan.io/api?module=contract&action=getabi&address=${answers.address}&apikey=${process.env.ARBITRUM_API_KEY_TOKEN}`
    )
    const { result } = await res.json()
    ...
    const res = await fetch(
      `https://api.snowtrace.io/api?module=contract&action=getabi&address=${answers.address}&apikey=${process.env.AVALANCHE_API_KEY_TOKEN}`
    )
    const { result } = await res.json()
