...
// ChainId 1 = mainnet, ChainId 137 = matic, 42161 = arbitrum, 43114 = avalanche, 10 = optimism, 250 = fantom, 8453 = base, 9745 = plasma, 56 = bsc
  ...
  constructor(config: Web3 | DSAConfig, chainId: ChainId = 1, options: Options = { skipChainIdValidation: false }) {
    ...
    if(!options.skipChainIdValidation) {
      this.config.web3.eth.getChainId().then((_chainId) => {
        ...
        if (![1, 137, 42161, 43114, 10, 250, 8453, 9745, 56].includes(chainId)) {
